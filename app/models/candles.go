...
func (c *Candle) Create() error {
	...
	_, err2 := stmt.Exec(c.Time.Format(time.RFC3339), c.Open, c.Close, c.High, c.Low, c.Volume)
	if err2 != nil {
		return err2
	}
	return err2
...
func (c *Candle) Save() error {
	...
	_, err2 := stmt.Exec(c.Open, c.Close, c.High, c.Low, c.Volume, c.Time.Format(time.RFC3339))
	if err2 != nil {
		return err2
	}
	return err2
