...
func GetSignalEventsByCounts(numberToLoadEvents int) *SignalEvents {
	...
	rows, err := DbConnection.Query(cmd, config.Config.ProductCode, numberToLoadEvents)
	if err != nil {
		return nil
	}
	defer rows.Close()
...
func GetSignalEventsAfterTime(theTime time.Time) *SignalEvents {
	cmd := fmt.Sprintf(`SELECT * FROM(
        SELECT time,product_code,side,price,size FROM %s
        WHERE DATETIME(time) >= ?
        ORDER BY time DESC)
        ORDER BY time ASC;`, tableNameSignalEvents)
	rows, err := DbConnection.Query(cmd, theTime.Format(time.RFC3339))
	if err != nil {
		log.Fatalln(err)
		return nil
	}
